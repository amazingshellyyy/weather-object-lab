...
const getCitys = function(obj){
    return obj.city.name;
}
console.log(getCitys(evanstonWeather));
...
function convertTimeStampToTime(unix){
    // conver unix into unix in milliseconds
    const unixInMilli = unix * 1000;
    // buid the date object
    const unixInMilliAsDate = new Date(unixInMilli);
    // to local Date string, forma
    return unixInMilliAsDate.toTimeString();
    
}
// convertTimeStampToTime(1403106000);

// convertTimeStampToTime(evanstonWeather.list.dt);


for (let i = 0; i < evanstonWeather.list.length; i++) {
    let time = convertTimeStampToTime(evanstonWeather.list[i].dt);
    // console.log("i", evanstonWeather.list[i].dt);
    console.log("time", convertTimeStampToTime(evanstonWeather.list[i].dt));
}
